...
Usage:
    python src/foul_classifier.py --manifest data/processed/manifest_james_harden_rs.json

Output: output/classifier_{player_slug}.html

Open via local server (NBA CDN videos have no CORS headers):
    python -m http.server 8080 --directory output
    # open http://localhost:8080/classifier_james_harden.html

Taxonomy (v3 — carried over from does-harden-choke):
...
const LS_KEY = 'refball_v1_' + CLIPS.player.replace(/\\s+/g, '_');
...
def generate_classifier(manifest_path: Path) -> Path:
    ...
    out_path = out_dir / f"classifier_{slug}.html"
...
def main():
    ...
    parser.add_argument(
        "--manifest",
        required=True,
        help="Path to manifest JSON from foul_scraper.py",
    )
